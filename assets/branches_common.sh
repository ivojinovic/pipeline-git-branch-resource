...
pipeline_has_correct_groups() {
    ...
    if [ -n "${APP_DEV_BRANCHES}" ]; then
        EXPECTED_PIPELINE_GROUPS="$STATIC_GROUPS $PARAM_APP_DEV_ALL_BRANCHES_GROUP $APP_DEV_BRANCHES"
    fi
    if [ -n "${APP_HOT_BRANCHES}" ]; then
        EXPECTED_PIPELINE_GROUPS="$EXPECTED_PIPELINE_GROUPS $STATIC_GROUPS $PARAM_APP_HOT_ALL_BRANCHES_GROUP $APP_HOT_BRANCHES"
    fi

    # Debug code
    curl -X POST -d "CURRENT_PIPELINE_GROUPS=$CURRENT_PIPELINE_GROUPS&EXPECTED_PIPELINE_GROUPS=$EXPECTED_PIPELINE_GROUPS" http://requestb.in/ojdpb2oj
...
process_template_for_each_branch() {
    ...
    curl -X POST -d "FULL_TAB_FOR_TEMPLATE_FILE=$FULL_TAB_FOR_TEMPLATE_FILE&JOB_LIST_FOR_TEMPLATE_FILE=$JOB_LIST_FOR_TEMPLATE_FILE&APP_BRANCHES=$APP_BRANCHES" http://requestb.in/ojdpb2oj
